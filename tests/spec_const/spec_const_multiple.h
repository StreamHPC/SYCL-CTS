...
template <typename T, typename via_kb>
class check_spec_const_multiple_for_type {
 public:
...
static void sc_run_test_core(util::logger &log) {
  using namespace spec_const_multiple;
  {
#if !SYCL_CTS_ENABLE_FULL_CONFORMANCE
    for_all_types<check_spec_const_multiple_for_type, via_kb>(
        get_spec_const::testing_types::types, log);
#else
    for_all_types_vectors_marray<check_spec_const_multiple_for_type, via_kb>(
        get_spec_const::testing_types::types, log);
#endif
    for_all_types<check_spec_const_multiple_for_type, via_kb>(
...
#if !SYCL_CTS_ENABLE_FULL_CONFORMANCE
    check_spec_const_multiple_for_type<sycl::half, via_kb> fp16_test{};
    fp16_test(log, "sycl::half");
#else
    for_type_vectors_marray<check_spec_const_multiple_for_type, sycl::half,
...
#if !SYCL_CTS_ENABLE_FULL_CONFORMANCE
    check_spec_const_multiple_for_type<double, via_kb> fp64_test{};
    fp64_test(log, "double");
#else
    for_type_vectors_marray<check_spec_const_multiple_for_type, double, via_kb>(
        log, "double");
#endif
  }
}
