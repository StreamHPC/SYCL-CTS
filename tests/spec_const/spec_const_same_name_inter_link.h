...
#include "spec_const_same_name_inter_link_helper.h"

namespace spec_const_same_name_inter_link {
...
class check_spec_const_same_name_inter_link_for_type {
 public:
...
static void sc_run_test_core(util::logger &log) {
  using namespace spec_const_help;
  {
    ...
    for_all_types<check_spec_const_same_name_inter_link_for_type,
                  sc_sn_il_config<tu_num>, via_kb>(
        get_spec_const::testing_types::types, log);
#else
    for_all_types_vectors_marray<check_spec_const_same_name_inter_link_for_type,
                                 sc_sn_il_config<tu_num>, via_kb>(
        get_spec_const::testing_types::types, log);
#endif
    for_all_types<check_spec_const_same_name_inter_link_for_type,
...
#if !SYCL_CTS_ENABLE_FULL_CONFORMANCE
    check_spec_const_same_name_inter_link_for_type<
        sycl::half, sc_sn_il_config<tu_num>, via_kb>
        fp16_test{};
    fp16_test(log, "sycl::half");
#else
    for_type_vectors_marray<check_spec_const_same_name_inter_link_for_type,
                            sycl::half, sc_sn_il_config<tu_num>, via_kb>(
        log, "sycl::half");
#endif
  }
...
#else
    for_type_vectors_marray<check_spec_const_same_name_inter_link_for_type,
                            double, sc_sn_il_config<tu_num>, via_kb>(log,
                                                                     "double");
#endif
  }
}
